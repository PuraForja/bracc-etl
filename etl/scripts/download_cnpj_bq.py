...
# Only SELECT columns the pipeline actually uses — minimizes BQ scan cost.
TABLES: dict[str, str] = {
    "empresas": (
...
# Rows per page when streaming results from BigQuery.
PAGE_SIZE = 100_000
...
def _download_table(
    ...
    *,
    ...
    """Stream a single BQ table to a CSV file."""
    ...
    sql = f"SELECT {columns} FROM `{DATASET}.{table}`"  # noqa: S608
    logger.info("Querying %s.%s (%d columns)...", DATASET, table, len(columns.split(",")))

    job = client.query(sql)
    rows_written = 0
    for i, chunk_df in enumerate(job.result().to_dataframe_iterable(max_results=PAGE_SIZE)):
...
def main(
    ...
    logger.info(
        ...
        len(selected), DATASET, billing_project,
    )
