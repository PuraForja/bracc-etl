...
## Documento Master Consolidado v26
> Gerado em 10/05/2026 ~04h00
...
## ORQUESTRADOR — FEATURES (10/05/2026)
...
MODO_TESTE=Y bash ~/Downloads/br-acc-novo/orchestrator.sh --force camara
...
- Barra de progresso por arquivo `[████░░░░░░] 4/18 (22%) Ano-2012.csv`
- Watchdog — avisa se pipeline parar por 3min sem atividade
- MODO_TESTE=Y — pula leitura Neo4j, uv sync e PNCP
...
[ ] Barra de progresso sobrescrever linha — Git Bash não suporta \r em scripts, tentar Windows Terminal futuramente
```
...
| 08/05 | world_bank fix + CNPJ fix + camara refactor + fix sessão única loader.py (1800x) |
| 09/05 | camara importação completa 3.79M + backup 9.9GB + MASTER v25 |
| 10/05 | barra progresso orchestrator + MODO_TESTE + watchdog + MASTER v26 |
...
v26 — 10/05/2026 ~04h00
CÂMARA: ✅ 3.79M expenses
ORCHESTRATOR: barra progresso + MODO_TESTE + watchdog
PRÓXIMO: senado + fix sessão única demais pipelines
