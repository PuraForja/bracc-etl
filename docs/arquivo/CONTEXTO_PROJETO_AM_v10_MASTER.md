# SISTEMA DE INTELIGÊNCIA POLÍTICA — AMAZONAS
## Documento Master Consolidado v10
> Gerado em 28/04/2026 — consolida v9 + sessão 28/04 madrugada
> Cole este arquivo no início de QUALQUER nova conversa para restaurar contexto completo.

---

## 🧠 INSTRUÇÃO OBRIGATÓRIA PARA O CLAUDE

Você está num projeto longo e técnico com Alberto (Rolim). Sua janela de contexto é limitada.

**Obrigações:**
1. Opere sempre em dois modos: **Arquiteto** (propõe soluções) + **Professor** (explica passo a passo)
2. Monitore o tamanho da conversa. Após 30-40 trocas, avise:
   > ⚠️ "Alberto, nossa conversa está ficando longa. Vou gerar um novo arquivo de contexto antes de continuar."
3. Gere novo `.md` atualizado seguindo este formato
4. Sempre conectar: **Dados → Diagnóstico → Problema → Ação → Monitoramento**
5. Nível técnico: intermediário — já programou, já usou Linux/Bash, absorve rápido mas precisa de orientação passo a passo
6. Sempre usar dados e exemplos do Amazonas quando possível
7. **REGRA:** Sempre propor testes rápidos ANTES de rodar pipelines longos
8. **REGRA:** Sempre agrupar múltiplos comandos bash em um único bloco
9. **REGRA:** Nunca rodar download e importação separadamente — baixar tudo primeiro, depois importar
10. **REGRA:** Preferir texto colado em vez de prints/imagens para economizar contexto de sessão
11. **REGRA:** Gerar novo contexto (.md) proativamente quando a conversa estiver longa
12. **REGRA NOVA:** Avisar Alberto com antecedência quando a conversa estiver ficando longa, ANTES de bloquear — ele precisa copiar o contexto antes de encerrar a sessão
13. **REGRA NOVA:** Quando algo demorar mais de 1 minuto sem output, é sinal de travamento — avisar imediatamente
14. **REGRA NOVA:** Sempre usar `grep -E "✅|⚠️" <log> && du -sh data/*` como comando padrão de diagnóstico rápido (1 comando só)
15. **REGRA NOVA:** Para copiar texto no Git Bash sem cancelar o processo: selecionar com mouse → clique direito → Copy. Ou ativar "Copy on select" em Options → Keys

---

## 👤 Perfil do Usuário

| Campo | Valor |
|---|---|
| Nome | Alberto (Rolim) |
| Contexto | Membro de partido político de oposição no Amazonas |
| Objetivo | Usar dados públicos para gestão política e tomada de decisão |
| Hardware | Xeon 2680 v4, 32GB RAM 2400, HD 2TB (~556GB livre) |
| SO | Windows 11 |
| Terminal | Git Bash |
| Python | `/c/Python314/python.exe` (v3.14.4) — uv usa este por padrão |
| uv | `~/.local/bin/uv` v0.11.7 |
| WSL2 | v2.6.3, configurado com 24GB RAM |

---

## 🎯 Objetivo do Projeto

Criar um **sistema de inteligência política para o Amazonas** com:
1. Coleta e visualização de dados públicos (saúde, educação, contratos, sanções, CNPJ, etc.)
2. Mapa interativo dos 62 municípios do AM com indicadores
3. Sistema de registro e acompanhamento de problemas por município
4. Dashboards para tomada de decisão política
5. Cruzamento de dados: CPF/CNPJ de políticos e empresas ligadas ao governo

---

## 🛠️ Infraestrutura

### Containers Docker
| Container | Porta | Status |
|---|---|---|
| bracc-neo4j | 7474/7687 | ✅ |
| br-acc-novo-api-1 | 8000 | ✅ |
| br-acc-novo-frontend-1 | 3000 | ✅ |
| etl | — | profile "etl" |

```bash
# Subir containers:
cd ~/Downloads/br-acc-novo && docker compose up -d
```

### Credenciais
| Sistema | Usuário | Senha |
|---|---|---|
| Neo4j | neo4j | changeme |
| Frontend | teste@bracc.com | senha123 |
| Invite code | — | rolim |

---

## ✅ STATUS ATUAL DOS DADOS (28/04/2026 — madrugada)

### Neo4j (banco atual)
| Tipo | Quantidade | Status |
|---|---|---|
| Company | 40.467.322 | ✅ CNPJs corretos |
| Partner | 17.774.658 | ✅ |
| Person | 1.090.627 | ✅ Candidatos TSE 2024 |
| Election | 16.707 | ✅ |
| SOCIO_DE | 18.783.607 | ⚠️ Incompleto (esperado 26.8M) |
| DOOU | 1.169.752 | ✅ |
| CANDIDATO_EM | 463.604 | ✅ |

### Backup do banco
```
Arquivo: neo4j-backup-20260426.tar.gz
Local:   C:\Users\Rolim\Downloads\
Tamanho: 9.4GB
```

### Downloads no HD — Status Detalhado

| Fonte | Tamanho | Status | Observação |
|---|---|---|---|
| cnpj | 28G | ✅ | 40M empresas — já importado no Neo4j |
| tse | 13G | ✅ | candidatos + doações 2024 — já importado |
| camara | 1.7G | ✅ | 18/18 anos (2009-2026) |
| senado | 71M | ✅ | 14/19 anos (2008-2021) — 2022-2026 retornam 404 |
| transparencia | 396M | ✅ | contratos 39k + emendas 857k (2024) — servidores 403 |
| leniency | 292K | ✅ | 146 acordos (arquivos existem mas 403 ao tentar reatualizar) |
| cpgf | 50M | ✅ | 15/16 meses (202511 com 403) |
| viagens | 7.7G | ✅ | 3.9M viagens (2020-2026) — Viagem/Pagamento/Passagem/Trecho por ano |
| tesouro_emendas | 61M | ✅ | emendas_tesouro.csv |
| siop | — | ✅ | 4.696.405 linhas — emendas 2020-2024 (939k linhas/ano) |
| cvm | 652K | ✅ | processo_sancionador.csv + acusado.csv |
| cvm_funds | 18M | ✅ | cad_fi.csv |
| icij | 667M | ✅ | Panama/Pandora Papers — nodes-entities/officers/intermediaries/relationships |
| ofac | 7.9M | ✅ | sdn.csv + add.csv + alt.csv |
| un_sanctions | 2.4M | ✅ | un_sanctions.json (1005 entidades) |
| opensanctions | 2.6G | ✅ | 4.1M entidades |
| holdings | 10M | ✅ | holding.csv.gz |
| renuncias | 510M | ✅ | 2020-2024 (5 anos) |
| querido_diario | — | ✅ | acts.jsonl (já existia) |
| senado_cpis | — | ✅ | (já existia) |
| siconfi | 452M+ | 🔄 | estados 2020-2024 OK — municípios rodando (5570 entidades × 5 anos, ~7h) |
| pncp | 13K | ⛔ | Descartado — 2340 combos, API retorna 204 vazio + timeouts |
| sanctions (CGU) | 0 | ❌ | 403 Forbidden — bloqueio CGU |
| cepim | 0 | ❌ | 403 Forbidden — bloqueio CGU |
| ceaf | 0 | ❌ | 403 Forbidden — bloqueio CGU |
| pep_cgu | 0 | ❌ | 403 Forbidden — bloqueio CGU |
| bcb | 0 | ❌ | 400 Bad Request — URL da API mudou |
| eu_sanctions | 0 | ❌ | 403 Forbidden |
| world_bank | 0 | ❌ | URLs mortas |
| camara_inquiries | — | ❌ | Falta dependência: `pip install '.[bigquery]'` |
| datasus | — | ❌ | Script de download não existia — criado nesta sessão (download_datasus.py) |
| comprasnet | — | N/A | É pipeline Neo4j, não downloader — usa dados do PNCP |

---

## 🔧 SCRIPTS GERADOS NESTA SESSÃO

### bracc_download_completo.sh (atualizado)
- Siconfi movido para **último** na fila
- Siconfi roda em 2 passes: `--states-only` primeiro, depois completo
- `--skip-existing` removido de renuncias e siconfi (não suportado)

### bracc_download_pendentes.sh
Script com apenas o que faltava após o download massivo:
```bash
bash ~/Downloads/bracc_download_pendentes.sh 2>&1 | tee ~/Downloads/download_pendentes.log
```
Ordem: renuncias → 403 CGU → siop → querido_diario → senado_cpis → camara_inquiries → siconfi (estados + municípios) → pncp

### download_datasus.py ⭐ NOVO
Script criado do zero usando **PySUS** para baixar CNES do FTP do DATASUS:
```bash
# Instalar dependência:
uv pip install pysus

# Rodar (AM primeiro, depois todos):
cd ~/Downloads/br-acc-novo/etl
uv run python scripts/download_datasus.py --output-dir ../data/datasus
uv run python scripts/download_datasus.py --output-dir ../data/datasus --uf AM  # só Amazonas
```
- Baixa grupo ST (Estabelecimentos) do CNES
- Salva parquets por UF/ano/mês em `data/datasus/raw/`
- Consolida em `data/datasus/cnes_all.csv` (formato esperado pelo pipeline)
- AM é sempre priorizado (baixado primeiro)
- Skip automático de arquivos já existentes

---

## 🔍 DESCOBERTAS DESTA SESSÃO

### SICONFI — o que é e como ajuda a detectar corrupção
- Sistema de Informações Contábeis e Fiscais do Setor Público Brasileiro (Tesouro Nacional)
- Todo município/estado obrigado por lei a enviar balanços contábeis
- Contém: receitas, despesas, dívidas, investimentos, transferências, gastos com pessoal
- **Uso para corrupção:** cruzar gastos declarados com contratos reais (PNCP), comparar municípios de porte similar, detectar desvios entre entrada e saída de recursos
- Dados são **autodeclarados** — corrupção sofisticada aparece nos cruzamentos
- `--annex AM` = tipo de relatório contábil "AM" (DCA-Anexo I-C), **NÃO** filtra pelo estado Amazonas
- Não há argumento de filtro por estado — baixa todos os 5570 municípios

### PNCP — descartado
- 2340 combos (195 janelas de datas × 12 modalidades)
- API retorna 204 (sem dados) para a maioria
- Timeouts frequentes (mod=4, mod=9, mod=11) de até 5 minutos cada
- Só salvou 13K após rodar por horas — descartado por ora

### Fontes CGU com 403 — bloqueio de IP confirmado
- Todas do mesmo servidor `dadosabertos-download.cgu.gov.br`
- Workarounds: baixar manualmente no browser, usar VPN, ou cadastrar chave de API em `portaldatransparencia.gov.br/api-de-dados`
- Fontes afetadas: sanctions (CEIS/CNEP), cepim, ceaf, pep_cgu, servidores, cpgf nov/2025

### camara_inquiries — falta dependência BigQuery
```bash
uv pip install '.[bigquery]'
# ou dentro do projeto:
cd ~/Downloads/br-acc-novo/etl && uv pip install google-cloud-bigquery
```

### datasus e comprasnet — situação real
- `datasus.py` e `comprasnet.py` em `src/bracc_etl/pipelines/` são **pipelines de importação Neo4j**, não downloaders
- Script de download do DATASUS **não existia** — criado nesta sessão
- comprasnet usa dados do PNCP (descartado)

### Loop de download automático — problema no Windows/Git Bash
- Script com loop `while` não funciona bem com `tee` no Git Bash (buffering)
- Solução: rodar o script simples múltiplas vezes manualmente

### Copiar texto no Git Bash sem matar processo
- Selecionar com mouse → clique direito → Copy
- Ou: Options → Keys → marcar "Copy on select"

---

## 🔧 CORREÇÕES ACUMULADAS (v1→v10)

### Correção 1 — URLs de download (runner.py)
Mirror Casa dos Dados adicionado como fallback.

### Correção 2 — Bug start_phase Fase 1 (cnpj.py)
`--start-phase` agora funciona corretamente.

### Correção 3 — Filtro ZIP — CAUSA RAIZ (cnpj.py) ✅
```python
files = [f for f in sorted(cnpj_dir.glob(pattern)) if f.suffix != ".zip"]
```

### Correção 4 — Empresas0.zip corrompido
Arquivo deletado.

### Correção 5 — Nginx timeout (volátil)
Pendente incorporar no Dockerfile.

### Correção 6 — bootstrap_all_contract.yml ✅
28 fontes atualizadas para `script_download`.

### Correção 7 — venv corrompido (lib64) ✅
```powershell
Remove-Item -Recurse -Force "C:\Users\Rolim\Downloads\br-acc-novo\etl\.venv"
```
```bash
cd ~/Downloads/br-acc-novo/etl && uv sync
```

### Correção 8 — wait_for_api no bootstrap ✅
Parâmetro `--api-url` adicionado em `scripts/run_bootstrap_all.py`.

### Correção 9 — renuncias e siconfi não aceitam --skip-existing ✅
```bash
uv run python scripts/download_renuncias.py --output-dir ../data/renuncias
uv run python scripts/download_siconfi.py --output-dir ../data/siconfi
```

---

## ⚠️ PROBLEMAS CONHECIDOS

| # | Problema | Status |
|---|---|---|
| 1 | SOCIO_DE incompletos (18.7M vs 26.8M) | ⚠️ Pendente |
| 2 | Scripts link_persons.cypher não existem | ⚠️ Pendente |
| 3 | Bug frontend grafo vazio para pessoas | ⚠️ Pendente |
| 4 | sanctions/cepim/ceaf/pep_cgu 403 CGU | ⚠️ Baixar manualmente ou usar API key |
| 5 | Senado 2022-2026 retornam 404 | ⚠️ Dados não disponíveis no formato antigo |
| 6 | Nginx timeout volátil | ⚠️ Pendente |
| 7 | Usuários perdidos ao reiniciar | ⚠️ Pendente |
| 8 | bcb URL API mudou (400) | ⚠️ Pendente investigar nova URL |
| 9 | world_bank URLs mortas | ⚠️ Pendente nova URL |
| 10 | pncp — 2340 combos, API retorna 204 | ⛔ Descartado por ora |
| 11 | camara_inquiries — falta google-cloud-bigquery | ⚠️ `uv pip install '.[bigquery]'` |
| 12 | siconfi_municipios — caiu com queda de internet (452M salvos) | 🔄 Retoma automaticamente |

---

## 💡 COMANDOS ÚTEIS

```bash
# ── Diagnóstico rápido de download ────────────────────────────────────
grep -E "✅|⚠️" ~/Downloads/download_pendentes.log && du -sh ~/Downloads/br-acc-novo/data/* | sort -h

# ── Rotina diária ─────────────────────────────────────────────────────
cd ~/Downloads/br-acc-novo && docker compose up -d

# ── Nginx timeout ─────────────────────────────────────────────────────
docker exec br-acc-novo-frontend-1 sh -c 'echo "server {
    listen 3000; root /usr/share/nginx/html; index index.html;
    location /assets/ { expires 1y; add_header Cache-Control \"public, immutable\"; }
    location / { try_files \$uri \$uri/ /index.html; add_header Cache-Control \"no-cache, no-store, must-revalidate\"; }
    location /api { proxy_pass http://api:8000; proxy_set_header Host \$host; proxy_set_header X-Real-IP \$remote_addr; proxy_read_timeout 300s; proxy_connect_timeout 300s; proxy_send_timeout 300s; }
}" > //etc/nginx/conf.d/default.conf && nginx -s reload'

# ── Recriar usuário ───────────────────────────────────────────────────
curl -X POST http://localhost:8000/api/v1/auth/register \
  -H "Content-Type: application/json" \
  -d "{\"email\":\"teste@bracc.com\",\"password\":\"senha123\",\"invite_code\":\"rolim\"}"

# ── Bootstrap (modo correto) ──────────────────────────────────────────
cd ~/Downloads/br-acc-novo && python3 scripts/run_bootstrap_all.py \
  --repo-root . \
  --compose-file docker-compose.yml \
  --api-url http://localhost:8000/health \
  --no-reset \
  --sources tse,camara,senado 2>&1 | tee ../bootstrap_test.log

# ── Pipeline CNPJ (20h) ───────────────────────────────────────────────
cd ~/Downloads/br-acc-novo/etl && source ~/.local/bin/env && \
  uv run bracc-etl run --source cnpj --neo4j-password "changeme" \
  --data-dir ../data --streaming 2>&1 | tee ../pipeline.log && \
  echo "✅ Finalizado em: $(date '+%Y-%m-%d %H:%M:%S')" | tee -a ../pipeline.log

# ── Neo4j ─────────────────────────────────────────────────────────────
docker exec bracc-neo4j cypher-shell -u neo4j -p changeme \
  "MATCH (n) RETURN labels(n)[0] as tipo, count(n) as total ORDER BY total DESC"

docker exec bracc-neo4j cypher-shell -u neo4j -p changeme \
  "MATCH ()-[r]->() RETURN type(r) as tipo, count(r) as total ORDER BY total DESC"

# ── Backup (PowerShell) ───────────────────────────────────────────────
# docker run --rm -v br-acc-novo_neo4j-data:/data -v C:/Users/Rolim/Downloads:/backup alpine tar czf /backup/neo4j-backup-YYYYMMDD.tar.gz /data

# ── Restaurar backup (PowerShell) ─────────────────────────────────────
# docker run --rm -v br-acc-novo_neo4j-data:/data -v C:/Users/Rolim/Downloads:/backup alpine tar xzf /backup/neo4j-backup-20260426.tar.gz -C /

# ── Limpar banco ──────────────────────────────────────────────────────
# docker stop bracc-neo4j && docker rm bracc-neo4j && docker volume rm br-acc-novo_neo4j-data
# cd ~/Downloads/br-acc-novo && docker compose up -d neo4j

# ── Ver logs de download ──────────────────────────────────────────────
ls ~/Downloads/bracc_logs/
grep -l 'FALHOU\|Error\|error' ~/Downloads/bracc_logs/*.log

# ── Verificar espaço por fonte ────────────────────────────────────────
du -sh ~/Downloads/br-acc-novo/data/* | sort -h

# ── Verificar arquivos baixados ───────────────────────────────────────
find ~/Downloads/br-acc-novo/data -name "*.csv" -o -name "*.json" -o -name "*.gz" | sort

# ── Continuar downloads pendentes ────────────────────────────────────
bash ~/Downloads/bracc_download_pendentes.sh 2>&1 | tee ~/Downloads/download_pendentes.log

# ── DATASUS (novo script) ─────────────────────────────────────────────
cd ~/Downloads/br-acc-novo/etl
uv pip install pysus
uv run python scripts/download_datasus.py --output-dir ../data/datasus
uv run python scripts/download_datasus.py --output-dir ../data/datasus --uf AM
```

---

## 📄 PATCH COMPLETO (correcoes_bracc_2026.patch)

```diff
diff --git a/etl/src/bracc_etl/pipelines/cnpj.py b/etl/src/bracc_etl/pipelines/cnpj.py
@@ -290,7 +290,7 @@
-        files = sorted(cnpj_dir.glob(f"extracted/{pattern}"))
+        files = [f for f in sorted(cnpj_dir.glob(f"extracted/{pattern}")) if f.suffix != ".zip"]
@@ -322,9 +322,9 @@
-        files = sorted(cnpj_dir.glob(f"extracted/{pattern}"))
+        files = [f for f in sorted(cnpj_dir.glob(f"extracted/{pattern}")) if f.suffix != ".zip"]
@@ -201,6 +201,9 @@
-        if use_bq:
+        if start_phase > 1:
+            logger.info("Skipping Phase 1 -- start_phase=%d", start_phase)
+        elif use_bq:
```

---

## 🔍 DESCOBERTAS IMPORTANTES (acumulado)

### API funciona via element_id
```bash
curl "http://localhost:8000/api/v1/entity/29025187000160"  # CNPJ sem formatação
curl "http://localhost:8000/api/v1/entity/by-element-id/4:...:47052943"
curl "http://localhost:8000/api/v1/entity/4:...:28475404/connections"
curl "http://localhost:8000/api/v1/graph/4:...:28475404"  # só Company
```

### Bug do frontend
`/api/v1/graph/{id}` só aceita Company, não Partner. Fix pendente no frontend.

### Caso Adail José Figueiredo Pinheiro
```
CPF: 772.677.962-49
Partner ID: 1eeb9bfb2fa9c3af
Person ID: 4:91a9add5-...:58722149 (após TSE)

Empresas AM:
  DAFIL PARTICIPACOES LTDA (29.025.187/0001-60) — holding — R$150k
  VIEIRALVES EMPREENDIMENTOS LTDA (19.362.619/0001-74) — imóveis — R$180k

Candidato a prefeito: 2016 e 2020 — Itacoatiara/AM (município 0225)
```

### 47 pipelines disponíveis
```
✅ script_download (28): tse, camara, senado, sanctions, transparencia...
⚠️  Google credentials (8): tse_bens, tse_filiados, dou, rais, stf, mides, caged, datajud
⚠️  Sem script (7): pgfn, tcu, ibama, bndes, inep, datasus*, transferegov
* datasus agora tem script criado nesta sessão
```

---

## 📋 FASES DO PROJETO

| Fase | Descrição | Status |
|---|---|---|
| 1 | Infraestrutura instalada | ✅ |
| 2 | Download CNPJ (~28GB) | ✅ |
| 3 | Pipeline ETL CNPJ | ✅ CNPJs corretos |
| 4 | Frontend com busca e grafo | ✅ Parcial |
| 5 | Corrigir Fase 1 estab_lookup | ✅ |
| 6 | Bootstrap corrigido (contrato + wait_for_api) | ✅ |
| 7 | TSE 2024 baixado e importado | ✅ |
| 8 | Câmara baixada (18 anos) | ✅ |
| 9 | Senado baixado (14 anos) | ✅ Parcial |
| 10 | Download de todas as fontes disponíveis | 🔄 ~85% concluído |
| 11 | siconfi municípios (~7h, caiu com internet) | 🔄 Retomar |
| 12 | camara_inquiries (falta bigquery) | ⏳ |
| 13 | datasus (script criado, ainda não rodado) | ⏳ |
| 14 | Fontes CGU 403 (baixar manualmente ou API key) | ⏳ |
| 15 | Importação de todas as fontes no Neo4j | ⏳ Próxima grande etapa |
| 16 | Corrigir bootstrap após 404 Senado | ⏳ |
| 17 | Corrigir bug frontend grafo pessoas | ⏳ |
| 18 | Corrigir SOCIO_DE incompletos | ⏳ |
| 19 | Nginx/usuários permanentes | ⏳ |
| 20 | PostgreSQL + tabelas AM | ⏳ |
| 21 | Metabase + dashboards | ⏳ |
| 22 | Mapa interativo 62 municípios AM | ⏳ |

---

## 📅 HISTÓRICO DE SESSÕES

| Data | O que foi feito |
|---|---|
| 20/04 | Infraestrutura instalada |
| 21/04 | Mirror Casa dos Dados — download CNPJ iniciado |
| 22/04 | Download CNPJ concluído |
| 24/04 | Correções URLs + start_phase |
| 25/04 | Causa raiz ZIP encontrada + patch gerado |
| 26/04 manhã | Pipeline CNPJ completo (~20h) com dados corretos |
| 26/04 tarde | Backup 9.4GB — diagnóstico SOCIO_DE — análise Adail Pinheiro |
| 26/04 noite | 47 pipelines mapeados — venv corrigido — contrato bootstrap corrigido |
| 27/04 manhã | TSE 2024 baixado e importado (1M pessoas, 5M doações) |
| 27/04 noite | Câmara (18 anos) baixada — bootstrap wait_for_api corrigido |
| 28/04 manhã | Senado baixado (14/19 anos) — bootstrap trava após 404 |
| 28/04 noite | Download massivo iniciado — 20+ fontes baixadas — siconfi rodando |
| 28/04 madrugada | **Scripts corrigidos** — siop/querido_diario/senado_cpis ✅ — siconfi 452MB (caiu com internet) — download_datasus.py criado — problemas CGU/pncp mapeados |

---

## ⚠️ CHECKLIST AO INICIAR NOVA SESSÃO

```
[ ] cd ~/Downloads/br-acc-novo && docker compose up -d
[ ] docker ps (3 containers rodando)
[ ] Reaplicar nginx timeout se necessário
[ ] Recriar usuário frontend se necessário
[ ] Verificar siconfi: ls -lh ~/Downloads/br-acc-novo/data/siconfi/
    → Deve ter dca_states_YYYY.csv (2020-2024) e dca_mun_YYYY.csv (2020-2024)
    → Se só tiver parcial, rodar: uv run python scripts/download_siconfi.py --output-dir ../data/siconfi
[ ] Instalar pysus e rodar download_datasus.py (ainda não rodado)
[ ] Tentar CGU manualmente no browser se bloqueio liberar
[ ] PRÓXIMA GRANDE ETAPA: importar todas as fontes no Neo4j via bootstrap
```

---

## 🗺️ PRÓXIMA SESSÃO — O QUE FAZER

1. **Verificar siconfi:** `ls -lh ~/Downloads/br-acc-novo/data/siconfi/` — retomar se ainda não terminou
2. **DATASUS:** Instalar pysus e rodar o novo script `download_datasus.py`
3. **camara_inquiries:** `uv pip install google-cloud-bigquery` e tentar de novo
4. **CGU 403:** Tentar baixar manualmente no browser ou cadastrar API key
5. **Fazer backup do Neo4j** antes de qualquer importação
6. **Iniciar importação** via bootstrap — prioridade: transparencia → renuncias → viagens → cpgf → siop → siconfi

---

*v10 — 28/04/2026 madrugada*
*Consolida v1→v9 + sessão 28/04 madrugada (correções scripts, siconfi parcial, datasus criado)*
